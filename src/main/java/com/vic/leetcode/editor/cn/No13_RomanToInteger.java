...
public class No13_RomanToInteger {
    ...
    class Solution {
        Map<Character, Integer> numberTable = new HashMap<Character, Integer>() {{
            put('I', 1);
            put('V', 5);
            put('X', 10);
            put('L', 50);
        ...
        public int romanToInt(String s) {
            int prev = numberTable.get(s.charAt(0));
            int result = prev;
            for (int i = 1; i < s.length(); i++) {
                Integer num = numberTable.get(s.charAt(i));
                if (prev < num) result -= prev * 2;
                result += num;
                prev = num;
            }
            return result;
